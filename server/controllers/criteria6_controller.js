import Criterion6Model from "../models/Criteria6.js";
import { handleFileUploadAndDatabase } from "../controllers/handle_file_uploads.js";

const submitCriterion6Data = async (req, res) => {
  try {
    const { department, academicYear, questions } = req.body;

    const existingEntry = await Criterion6Model.findOne({ department });
    if (existingEntry) {
      return res
        .status(400)
    ...
    const newData = new Criterion6Model({
      department,
      academicYear,
