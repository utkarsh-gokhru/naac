...
const deleteExistingFiles = async (data) => {
  ...
  try {
    ...
    return deletePromises.map((promise) => ({
    ...
    try {
      ...
      for (const fieldName in files) {
        ...
        try {
          ...
          console.error(
            `Error uploading or getting download URL for ${fieldName}:`,
            error
          );
        }
      }
      ...
      let existingData = await Criteria3Model.findOne({
      ...
      if (existingData) {
        ...
        if (updateResult.success) {
          try {
            ...
            console.log("hahaahhaha le's see !");
        ...
        console.log("what in the world is this ? ");
